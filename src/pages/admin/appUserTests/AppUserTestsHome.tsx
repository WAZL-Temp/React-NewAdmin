...
export default function AppUserTestsHome() {
  const navigate = useNavigate();
  const { t } = useTranslation();
  const appUserTestsService = AppUserTestsService();
  const query = useHomeQuery(appUserTestsService);
      ...
      <div className="flex flex-col p-4">
        <div className="flex flex-col border-none mb-10">
          ...
          {topData && topData.length > 0 ? (
            ...
            <p className="text-center text-[var(--color-dark)]">No Data available</p>
          )}
...
const ItemSlider = ({ user }: { user: AppUserTest }) => {
  const navigate = useNavigate();
      ...
      <div className="w-full gap-2 items-center justify-center px-2 py-4 lg:px-4 lg:py-4">
        <div className="flex flex-col overflow-hidden border  border-[var(--color-gray)] p-5 rounded-lg shadow-xl hover:shadow-2xl transform hover:scale-105 transition-all duration-500">
          ...
          <div className="text-center">
            ...
            {user.name && (<h6 className="text-[var(--color-dark)]] text-sm mb-3">{user.name}</h6>)}
...
const ItemCard = ({ user }: { user: AppUserTest }) => {
  const navigate = useNavigate();
  const { t } = useTranslation();

  const handleUserView = (userid: number) => {
    navigate(`/appUserTests/${userid}`)
  }
