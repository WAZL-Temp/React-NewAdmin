...
export default function SeosHome() {
  const navigate = useNavigate();
  const { t } = useTranslation();
  const seosService = SeosService();
  const query = useHomeQuery(seosService);
...
const ItemSlider = ({ item }: { item: Seo }) => {
  const navigate = useNavigate();
