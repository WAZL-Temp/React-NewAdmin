...
const typeName= "seo";
    ...
    const dtRef = useRef<DataTable<Seo[]>>(null);
    // search
    ...
    const query = useListQuery<Seo>(seosService);
    const {
...
if (roleData && hasAccess(roleData, "Export")){
    items.push({
        ...
        command: () => exportToExcel(seosService, globalFilterValue || '', 'Seo')
    });
}
    ...
    const actionBodyTemplate = useCallback((rowData: Seo, openItem: (item: Seo, action: string) => void) => {
        return (
    ...
    return (
        <div className='relative h-screen flex flex-col overflow-auto'>
            <div className="flex justify-between items-center m-1">
                <h1 className="font-bold text-[16px] lg:text-xl ml-2">{t("seos.form_detail.fields.modelname")}</h1>
            </div>
 {query.isLoading ? (
                <Loader />
            ) : (
            ...
            <div className="flex mx-2 flex-wrap justify-between items-center gap-3 border text-[var(--color-dark)] border-[var(--color-border)] rounded-md p-1 lg:my-1">
                ...
                <div className="hidden lg:flex items-center space-x-2 flex-wrap  bg-[var(--color-white)] text-[var(--color-dark)]">
                    ...
                    <Button
                        ...
                        onClick={() => exportToExcel(seosService, globalFilterValue || '', 'Seo')}
...
filterElement={
<InputText
value={query.tableSearch.searchRowFilter?.title || ''}
className="w-full bg-[var(--color-white)] text-[var(--color-dark)] border border-[var(--color-border)] rounded-md p-[5px]"
...
filterElement={
<InputText
value={query.tableSearch.searchRowFilter?.description || ''}
className="w-full bg-[var(--color-white)] text-[var(--color-dark)] border border-[var(--color-border)] rounded-md p-[5px]"
...
<Tooltip className="text-xs font-semibold hide-tooltip-mobile" target={`#tooltip-description-${rowIndex}`} content={rowData.description} showDelay={200} position="top" />
 ...
 />)} 
{visibleColumns.includes('keyWords') && (
<Column field="keyWords" header={t("seos.columns.fields.keyWords")} sortable filter
headerStyle={{backgroundColor: "var(--color-primary)", color: "var(--color-white)", textAlign: "center" }}
style={{width: "200px", backgroundColor: "var(--color-white)" }}
filterElement={
<InputText
value={query.tableSearch.searchRowFilter?.keyWords || ''}
className="w-full bg-[var(--color-white)] text-[var(--color-dark)] border border-[var(--color-border)] rounded-md p-[5px]"
...
<Tooltip className="text-xs font-semibold hide-tooltip-mobile" target={`#tooltip-keyWords-${rowIndex}`} content={rowData.keyWords} showDelay={200} position="top" />
 ...
 />)} 
{visibleColumns.includes('imageUrl') && (
<Column field="imageUrl" header={t("seos.columns.fields.imageUrl")} sortable filter
headerStyle={{backgroundColor: "var(--color-primary)", color: "var(--color-white)", textAlign: "center" }}
style={{width: "200px", backgroundColor: "var(--color-white)" }}
filterElement={
<InputText
value={query.tableSearch.searchRowFilter?.imageUrl || ''}
className="w-full bg-[var(--color-white)] text-[var(--color-dark)] border border-[var(--color-border)] rounded-md p-[5px]"
...
filterElement={
<InputText
value={query.tableSearch.searchRowFilter?.url || ''}
className="w-full bg-[var(--color-white)] text-[var(--color-dark)] border border-[var(--color-border)] rounded-md p-[5px]"
...
<Tooltip className="text-xs font-semibold hide-tooltip-mobile" target={`#tooltip-url-${rowIndex}`} content={rowData.url} showDelay={200} position="top" />
 ...
 />)} 
{visibleColumns.includes('mainUrl') && (
<Column field="mainUrl" header={t("seos.columns.fields.mainUrl")} sortable filter
headerStyle={{backgroundColor: "var(--color-primary)", color: "var(--color-white)", textAlign: "center" }}
style={{width: "200px", backgroundColor: "var(--color-white)" }}
filterElement={
<InputText
value={query.tableSearch.searchRowFilter?.mainUrl || ''}
className="w-full bg-[var(--color-white)] text-[var(--color-dark)] border border-[var(--color-border)] rounded-md p-[5px]"
